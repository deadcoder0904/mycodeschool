...
string getUptoSecondPrecision(int i){
	stringstream value;
	if(i < 9)
		value<<"0"<<i+1;
	else value<<i+1;
...
int main()
{
	...
	while(t--)
	{
		...
		REP(i,s1.length())
			lookup[getUptoSecondPrecision(i)] = s1.at(i);
		REP1(i,s2.length(),2){
				stringstream lookup_key;
				lookup_key<<s2.at(i)<<s2.at(i+1);
				cout<<lookup[lookup_key.str()];
		}
		cout<<endl;
	}	
	return 0;
